<template>
	<view class="content-trend">
				...
				<div class="time-items">
					<view class="time-items-box">
						<view class="left RTI-box" :class="items.code === '001' ? 'RTI-boxIte' : ''" @tap="clikeTimeTab(items.code)" v-for="(items, index) in tabList" :key="index">
							<view :class="clickItemsIndex === items.code ? 'RTI-box-top active-tapRTI' : 'RTI-box-top'"><image :src="Image" v-if="index==4"></image>{{ items.text }}</view>
						</view>
				...
				<view class="tongji-box">
					<view class="item-tj">
						<scroll-view class="scroll-view_H" scroll-x="true">
							<view class="tongji-item left" v-for="(item, index) in i18n.flowList" :key="index">
								<view class="items-YY">
									<p class="tj-num">{{ item.code }}</p>
									<p class="tj-text">{{ item.text }}</p>
								</view>
								<view class="line-YY" v-if="index !== flowList.length - 1"></view>
							</view>
						</scroll-view>
					</view>
				</view>
							...
							<div class="screen-date Middle">
								<p class="vs">
									<picker @change="leftScreenDataChange" :value="0" :range="i18n.leftScreenData" range-key="text">
										...
										<span class="screen-image1"><image :src="downarrow"></image></span>
									</picker>
									VS
									<picker @change="rightScreenDataChange" :value="0" :range="i18n.leftScreenData" range-key="text">
		...
		<!-- 自定义时间组件 -->
		<customTime v-if="GET_ALITTLE.customTime.show" v-on:stomTime="stomTime"></customTime>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		stomTime:function(stomTime){
			...
			this.Trenddata(stomTime[0],stomTime[1]);
			...
			if (uni.getStorageSync('landRegist')) {
				...
				uni.request({
					...
					success: (response) => {
						...
						this.i18n.flowList[0].code=this.timess.infoCount;
						this.i18n.flowList[1].code=this.timess.likeCount;
						this.i18n.flowList[2].code=this.timess.followCount;
						this.i18n.flowList[3].code=this.timess.shareCount;
						this.i18n.flowList[4].code=this.timess.msgCount;
						this.i18n.flowList[5].code=this.timess.telCount;
						this.i18n.flowList[6].code=this.timess.inquCount;
						this.i18n.flowList[7].code=this.timess.chatCount;
						this.i18n.flowList[8].code=this.timess.orderCount;
					},
					fail: (error) => {
						uni.hideLoading(); // 隐藏 loading
						uni.showToast({
		...
		leftScreenDataChange: function(e) {
			...
			this.i18n.leftScreenData.map((items, index) => {
				if (String(index) === String(e.target.value)) {
					...
					if(items.text=='访问量'){
						this.Chart(this.Starttiming,this.Closingtime,this.trendKanBanText.trendLeft)//图表数据
		...
		rightScreenDataChange: function(e) {
			...
			this.i18n.leftScreenData.map((items, index) => {
				if (String(index) === String(e.target.value)) {
					...
					this.updateUcharts(this.LineA);
					if(items.text=='访问量'){
						this.Chart(this.Starttiming,this.Closingtime,this.trendKanBanText.trendLeft,this.trendKanBanText.trendRight)//图表数据
					}else if(items.text=='点赞数'){
						this.Chart(this.Starttiming,this.Closingtime,this.trendKanBanText.trendLeft,this.trendKanBanText.trendRight)//图表数据
					}else if(items.text=='收藏数'){
						this.Chart(this.Starttiming,this.Closingtime,this.trendKanBanText.trendLeft,this.trendKanBanText.trendRight)//图表数据
...
.time-items-box>view:nth-of-type(5){

<template>
	<view class="find-Purchase">
		<view v-for="(item,index) in arr" :key="index">
			<view class="find-Purchase-head" v-if="item.userAuth==2" @tap="gotocomp(item.userId)">
...
<!-- 						<view v-if="item.purcState==1&&item.type=='purc'">采购中</view>
			...
			<view class="myproduct-list-edit" v-if="item.type!=='compService'&&item.type!=='invest'&&item.type!=='purc'">
				...
				<!-- 点赞 -->
				<view class="product-book-BA" v-if="likes==0" @tap="dianzan(item.id,item.type)">
					<image :src="zan"></image>
					<span>{{likenum||'0'}}</span>
				</view>
			...
			<view class="myproduct-list-edit" v-if="item.type=='purc'">
				...
				<view class="product-book-BAs" v-if="likes==1" @tap="qudianzan(item.id,item.type,item.likeCount)">
					<image :src="zanss"></image>
					<span>{{likenum||'0'}}</span>
				</view>
				<!-- 收藏 -->
				...
				<!-- 分享 -->
				<view class="product-book-BAs right shou" v-if="item.isShare==0" @tap="tiaozhuan(item)">
					<image :src="fen"></image>
					<span>{{item.isShare}}</span>
				</view>
				<view class="product-book-BAs right shou" v-if="item.isShare==1" @tap="tiaozhuan(item)">
					<image :src="fens"></image>
					<span>{{item.isShare}}</span>
				</view>
			</view>

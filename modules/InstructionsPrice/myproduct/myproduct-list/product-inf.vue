<template>
	...
	<view class="myproduct">
		...
		<view class="myproduct-list" @tap="gotomyproducts(pro.instrucId)" v-for="(pro,index) in shu" :key="index">
			...
			<view style="position: relative;" v-if="pro.bannerImg!==''">
				<view class="myproduct-list-name nametwo">{{pro.instrucTitle}}</view>
				<view class="myproduct-list-time">更新于 {{pro.updateTime}}</view>
				<view class="myproduct-list-pick">
					<image :src="pro.bannerImg" mode=""></image>
				</view>
				<view class="myproduct-list-edit edittwo">
					<!-- 看过 -->
					<view class="product-book-BA one">
						<image :src="look"></image>
						<span>{{pro.pv}}</span>

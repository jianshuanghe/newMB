				...
				<div class="time-items">
					<view class="time-items-box">
						<view class="left RTI-box" :class="items.code === '001' ? 'RTI-boxIte' : ''" @tap="clikeTimeTab(items.code)" v-for="(items, index) in tabList" :key='index'>
							<view :class="clickItemsIndex === items.code ? 'RTI-box-top active-tapRTI' : 'RTI-box-top'"><image :src="Image" v-if="index==3"></image>{{items.text}}</view>
						</view>
				...
				<view class="tongji-box">
					<view class="item-tj">
						<scroll-view class="scroll-view_H" scroll-x="true">
							<view class="tongji-item left" v-for="(item, index) in i18n.leftScreenData" :key="index">
								<view class="items-YY">
									<p class="tj-num">{{ item.code }}</p>
									<p class="tj-text">{{ item.text }}</p>
								</view>
								<view class="line-YY" v-if="index !== leftScreenData.length - 1"></view>
							</view>
						</scroll-view>
					</view>
				</view>
			...
			if (uni.getStorageSync('landRegist')) {
				...
				uni.request({
					...
					success: response => {
						...
						this.timess = response.data.content;
						this.i18n.leftScreenData[0].code = this.timess.infoCount;
						this.i18n.leftScreenData[1].code = this.timess.likeCount;
						this.i18n.leftScreenData[2].code = this.timess.followCount;
						this.i18n.leftScreenData[3].code = this.timess.shareCount;
						this.i18n.leftScreenData[4].code = this.timess.telCount;
					},
					fail: error => {
		...
		leftScreenDataChange: function(e) {
			...
			this.i18n.leftScreenData.map((items, index) => {
				if (String(index) === String(e.target.value)) {
					...
					this.trendKanBanText.trendLeft = e.target.value; // 左侧选额code
					console.log(this.trendKanBanText.trendLeft)
		...
		rightScreenDataChange: function(e) {
			...
			this.i18n.leftScreenData.map((items, index) => {
				if (String(index) === String(e.target.value)) {
					...
					this.trendKanBanText.trendRight = e.target.value; // 右侧选额code
...
<style>
	...
	.time-items-box>view:nth-of-type(4){
		margin-left: 160upx;
	}
	.active-tapRTI {

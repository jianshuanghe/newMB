					...
					<div class="trend">
						<div class="title-box">
							...
							<div class="screen-date Middle">
								<p class="vs">
									<picker @change="leftScreenDataChange" :value="0" :range="i18n.flowList2" range-key="text">
										<span class="screen-item1">{{ trendKanBanText.trendLeftText }}</span>
									</picker>
									VS
									<picker @change="rightScreenDataChange" :value="0" :range="i18n.flowList2" range-key="text">
...
export default {
	...
	methods: {
		...
		leftScreenDataChange: function(e) {
			console.log('picker发送选择改变，携带值为', e.target.value);
			this.i18n.flowList2.map((items, index) => {
				if (String(index) === String(e.target.value)) {

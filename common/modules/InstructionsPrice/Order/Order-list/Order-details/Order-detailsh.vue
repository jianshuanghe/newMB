	...
	<view class="Order-details">
		...
		<view class="Order-details-F">
			...
			<view class="Order-details-F-ji" v-for="(item,index) in thid" :key="index">
				...
				<view v-for="(ite,index) in item.dataList" v-if="item.iconTitle === '样式选择'" style="clear: both;">{{ite.content3}}</view>
				<view v-for="(ite,index) in item.dataList" v-if="item.iconTitle === '采购数量'">{{ite.key}}: {{ite.count}}</view>
				<view v-for="(ite,index) in item.dataList" v-if="item.iconTitle === '添加照片'">
					<image :src="ite.imgUrl" class="images"></image>
				</view>
...
<!-- 			<view class="Order-details-H-thr">
		...
		<view class="Order-details-Z">
			<view class="Order-details-Z-header">
				<view>订单总额</view>
				<view>￥{{list.orderYfMoney+list.orderSpMoney | numFilter | numFilter}}</view>
			</view>
			<view class="Order-details-Z-foot" v-if="list.orderState == 0">
				...
				<view class="Order-details-Z-twow" @tap="guanbis()">关闭交易</view>
			</view>
			<!-- 已关闭 -->
			<view class="Order-details-Z-foot" v-if="list.orderState == 4">
				<view class="Order-details-Z-onea" @tap="phones()">
					<image :src="phone"></image><span>联系</span><span>|</span>
				</view>
				<view class="Order-details-Z-two" @tap="deleadd()">删除订单</view>
				<view class="Order-details-Z-tht" @tap="gotoAgain(list)">再来一单</view>
			</view>
			<!-- 待发货 -->
			<view class="Order-details-Z-foot" v-if="list.orderState == 1">
				...
				<view class="Order-details-Z-thr" @tap="remind()">提醒发货</view>
			</view>
			<!-- 已完成 -->
			<view class="Order-details-Z-foot" v-if="list.orderState == 3">
				<view class="Order-details-Z-one" @tap="phones()">
					<image :src="phone"></image><span>联系</span><span>|</span>
				</view>
				<view class="Order-details-Z-thra" @tap="gotoAgain(list)">再来一单</view>
			</view>
			<!-- 待收货 -->
				...
				if (uni.getStorageSync('landRegist')) {
					...
					uni.request({
						...
						success: (response) => {
							...
							console.log(this.moneyzong+this.moneyyun)
...
<style>
	...
	.Order-details-Z-onea>span:nth-of-type(1){
		display: block;
		float: left;
		margin-left: 10upx;
	}
	.Order-details-Z-onea>span:nth-of-type(2){
		color: #E2E2E2;
		display: block;
		float: right;
	}
